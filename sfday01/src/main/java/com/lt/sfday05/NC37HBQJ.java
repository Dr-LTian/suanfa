...
思路:将所有区间的起点进行排序 排序后第一个区间为最小起点区间 将此区间放入result的list中，
遍历剩余排序过的区间，如果起点小于等于结果中最后一个区间的终点 说明区间重叠 更新终点
如果起点大于最后一个区间的终点 不重合 将这个区间加入list 重复此比较过程 遍历完所有的区间 最终合并后的区间就是result的结果
 */
