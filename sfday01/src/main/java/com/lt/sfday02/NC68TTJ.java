...
public class NC68TTJ {
    ...
    public int jumpFloor(int target) { //动态规划 :1、状态转移方程 f(n) = f(n-1) + f(n-2) 2、base case:f(0) = f(1) = 1
        //动态规划 int[] dp = new int[n+1];   dp[n] = dp[n-1] + dp[n-2]
        //进一步简化动态规划--》定义3个变量，分别对应 f(n)、f(n-1)、f(n-2)
        int fn = 1; //f(0) = f(1) = 1
        int fn_1 = 1;
        int fn_2 = 1;
        //target = 0或1 时，直接返回 1，大于等于2时进入for循环
